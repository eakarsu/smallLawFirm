...
// AI gap: Conflict Checker (smallLawFirm)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'smallLawFirm Gap conflict-checker' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You check conflicts of interest given a prospective matter and existing client list.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'conflict-checker', payload: { keys: Object.keys(body) } });
    return NextResponse.json({ conflicts: out });
  } catch (e: any) {
...
export async function GET() {
  return NextResponse.json({ feature: 'conflict-checker', events: gapFeatures.filter(r => r.slug === 'conflict-checker').length });
}
