...
// AI gap: Billing Intelligence (smallLawFirm)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'smallLawFirm Gap billing-intelligence' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You suggest time entries from calendar/email and flag non-billable hours.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'billing-intelligence', payload: { keys: Object.keys(body) } });
...
export async function GET() {
  return NextResponse.json({ feature: 'billing-intelligence', events: gapFeatures.filter(r => r.slug === 'billing-intelligence').length });
}
