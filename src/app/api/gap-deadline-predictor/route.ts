...
// AI gap: Deadline Predictor (smallLawFirm)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'smallLawFirm Gap deadline-predictor' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You predict and flag critical deadlines (SoL, trial dates, filing deadlines).";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'deadline-predictor', payload: { keys: Object.keys(body) } });
    return NextResponse.json({ deadlines: out });
  } catch (e: any) {
...
export async function GET() {
  return NextResponse.json({ feature: 'deadline-predictor', events: gapFeatures.filter(r => r.slug === 'deadline-predictor').length });
}
